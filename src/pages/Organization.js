...
const fetchData = useCallback(async () => {
  try {
    ...
    const orgRes = await axios.get(`${BASE_URL}/api/organization`);
    ...
    const settingsRes = await axios.get(`${BASE_URL}/api/organization/settings`);
...
const serviceFields = [
  ...
  for (const setting of settingsToSave) {
    ...
    try {
      await axios.post(`${BASE_URL}/api/organization/settings`, {
 ...
 // Add metadata
    ...
    const orgRes = await axios.post(`${BASE_URL}/api/organization`, payload);
  ...
  const saveAdditionalSettings = async () => {
    ...
    for (let i = 0; i < valid.length; i++) {
      ...
      try {
        ...
        const res = await axios.post(`${BASE_URL}/api/organization/settings`, payload);
 ...
 const handleEditSave = async () => {
  ...
  try {
    ...
    await axios.post(`${BASE_URL}/api/organization/settings`, payload);
