...
function ServerMaster({ sidebarOpen }) {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${BASE_URL}/api/server`);
      setData(res.data);
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editId) {
        await axios.put(`${BASE_URL}/api/server/${editId}`, form);
      } else {
        await axios.post(`${BASE_URL}/api/server`, form);
      }
      setShowModal(false);
      fetchData();
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this server?")) {
      try {
        await axios.delete(`${BASE_URL}/api/server/${id}`);
        setShowModal(false);
        fetchData();
      } catch (err) {
